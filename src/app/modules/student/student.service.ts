...
const updateStudent = async (
  ...
  /* 
    const name = {
      firstName: "Mezba", <---- update this field
      lastName: "Persian", 
    }
  **/
  ...
  if (name && Object.keys(name).length > 0) {
    Object.keys(name).forEach(key => {
      ...
      (updatedStudentData as any)[nameKey] = name[key as keyof typeof name]; // `name.firstName : "Mezba"`
    });
  }
  ...
  if (guardian && Object.keys(guardian).length > 0) {
    Object.keys(guardian).forEach(key => {
      ...
      // updatedStudentData --> object create --> guardian : {motherContactNo: '0177777777'}
    });
  }
